      ...
      <div class="modal-card-custom animate-fade-in">
        ...
        <div class="modal-body-custom">
          ...
          <div class="mb-3">
            ...
            <input type="number" id="restockAmt" v-model.number="restockAmount" min="1" class="form-control-style" placeholder="Jumlah unit..." />
          </div>
        </div>

        <div class="modal-footer-custom border-top">
          <button @click="$emit('close')" class="btn-cancel">Batal</button>
          <button @click="handleConfirm" class="btn-confirm">Simpan Stok Baru</button>
        </div>
      </div>
    </div>
  </transition>
